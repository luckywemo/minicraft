...
describe('usePainLevel', () => {
  ...
  it('matches the page implementation in pain page', () => {
    ...
    // The pain page at /assessment/steps/pain/page.tsx uses the usePainLevel 
    // hook to manage pain level selection
    const mockUpdateResult = vi.fn();
    (useAssessmentContext as any).mockReturnValue({
      state: { result: {} },
      updateResult: mockUpdateResult
    ...
    act(() => {
      ...
      // In the page, this happens when the user selects an option like "severe"
      result.current.setPainLevel('severe');
    });

    // Verify the context was updated as expected, similar to how it would be
    // when a user interacts with the pain page
